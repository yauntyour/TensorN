...
namespace TensorN
{
    namespace cuda
    {
        ...
        void outer(const CudaTensor<T>& A, const CudaTensor<T>& B, CudaTensor<T>& C)
        {
            ...
            cudaMemset(C.device_ptr(), 0, m * n * sizeof(T));
            outer_kernel<<<grid, block>>>(A.device_ptr(), B.device_ptr(), C.device_ptr(), m, n);
        ...
        void axpy(T alpha, const CudaTensor<T>& x, CudaTensor<T>& y)
        {
            ...
            axpy_kernel<<<gs, bs>>>(x.device_ptr(), y.device_ptr(), alpha, n);
            CHECK_CUDA_ERROR(cudaGetLastError());
        }
        ...
        void diag(const CudaTensor<T>& A, CudaTensor<T>& C)
        {
            ...
            size_t n = A.shape()[0];
            size_t bs = 256, gs = (n + bs - 1) / bs;
            diag_kernel<<<gs, bs>>>(A.device_ptr(), C.device_ptr(), n, A.shape()[1]);
            CHECK_CUDA_ERROR(cudaGetLastError());
        }
        ...
        void diag_matrix(const CudaTensor<T>& v, CudaTensor<T>& C)
        {
            ...
            diag_matrix_kernel<<<gs, bs>>>(v.device_ptr(), C.device_ptr(), n, n);
            CHECK_CUDA_ERROR(cudaGetLastError());
        }
